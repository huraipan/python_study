...
class Stock:
    """retrieves data on the historical prices and trading volumes of a given stock, and then plots this data in a graph."""
    def __init__(self, code, year, month, day):

        if not isinstance(code, str) or len(code) != 6:
            raise ValueError("Invalid code: code must be a string of length 6")
        ...
        if date > datetime.datetime.today():
            raise ValueError("Invalid date: future dates are not allowed")
